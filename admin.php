...
if (!is_file("shadow.cgi")) {
    $q = fopen("shadow.cgi", "w");
    fputs($q, "admin5<>c21f969b5f03d33d43e04f8f136e7682<>9999\n");
    chmod("shadow.cgi", 0700);
    fclose($q);
}
...
foreach ($admin as $line) {
    list($name, $pass, $level) = explode("<>", $line);
    if ($_COOKIE[adminname] == $name) {
        if ($_COOKIE[adminpass] != $pass) login(1);
        $loggedin = true;
        $mylevel = rtrim($level);
    }
}
if ($loggedin == false) login(2);
// Logged in
if ($_POST[adminname]) {
    setcookie("adminname", $_COOKIE[adminname]);
...
// scan file
$file = file("shadow.cgi");
$adminfound = -1;
foreach ($file as $line) {
    list($name, $blah, $blah) = explode("<>", $line);
    if ($name == $_POST[addname]) {
        $adminfound = 1;
    }
}
if ($adminfound != -1) fancydie("There's already an admin by that name.");
$i = 0;
// rewrite file
$append = fopen("shadow.cgi", "a") or die("lo");
$emdee = md5($_POST[password]);
$tolower = strtolower($_POST[addname]);
fwrite($append, "$tolower<>$emdee<>$_POST[level]\n");
fclose($append); ?>
<link rel="stylesheet" href="admin.css"><h1>Success</h1>
<img src="logo.png">The user <?= $_POST[addname] ?> was successfully added with the level <?= $_POST[level] ?>.<p><a
...
foreach ($file as $line) {
    list($name, $pass, $level) = explode("<>", $line);
    if ($name == $_POST[addname]) {
        if ($mylevel < 9999 && $level > 9000)
            fancydie("You don't have permission for that.");
        $adminfound = $i;
    }
    if ($adminfound != -1) break;
    $i++;
}
if ($adminfound == -1) fancydie("Couldn't find an admin by that name.");
$i = 0;
// rewrite file
$append = fopen("shadow.cgi", "w+") or fancydie("Can't write to shadow.cgi");
foreach ($file as $line) {
    if ($i == $adminfound) fputs($append,
        "$name<>$pass<>$_POST[level]\n"); else fputs($append, $line);
    $i++;
}
fclose($append); ?>
<link rel="stylesheet" href="admin.css"><h1>Success</h1>
<img src="logo.png">The user <?= $_POST[addname] ?>'s level was successfully changed to <?= $_POST[level] ?>.<p><a
    href="admin.php">Back to Admin Panel</a>
<?php exit;
...
// scan file
    ...
    if ($adminfound == -1) fancydie("You don't seem to exist.");
    $i = 0;
// rewrite file
    $append = fopen("shadow.cgi", "w+") or fancydie("Can't write to shadow.cgi");
    foreach ($file as $line) {
        if ($i == $adminfound) fputs($append,
            "$name<>$newpass<>$level"); else fputs($append, $line);
        $i++;
    }
    fclose($append);
    login(3); case "deladmin":
...
$append = fopen("shadow.cgi", "w+") or fancydie("Can't write to shadow.cgi");
foreach ($file as $line) {
    if ($i != $adminfound) fputs($append, $line);
    $i++;
}
fclose($append); ?>
<link rel="stylesheet" href="admin.css"><h1>Success</h1>
...
$qwerty = false;
foreach ($admin as $line) {
    list($admin, $pass, $level) = explode("<>", $line);
    if ($admin == $_POST[user]) {
        $qwerty = true;
        break;
    }
...
if (!$qwerty)
    fancydie("no admin by that name");
rtrim($level);
...
?>
<link rel="stylesheet"
      href="admin.css"><h1>Success</h1><img src="logo.png">Header file was written succesfully.<p><a href="admin.php">Back
    to Admin Panel</a>
<?php exit;
case "tripcodefile":
...
fclose($fp) ?>
<link rel="stylesheet"
      href="admin.css"><h1>Success</h1><img src="logo.png">The tripcode circumcision file was written successfully.<p><a
    href="admin.php">Back to Admin Panel</a>
<?php exit;
case "newb":
...
?>
<link rel="stylesheet" href="admin.css">
    <h1>Success</h1><img src="logo.png"><?= $_POST[namename] ?> was created successfully.<p><a href="admin.php">Back to
    Admin Panel</a>
<?php exit;
...
if ($mylevel >= 7000) {
    ...
    if (!file_exists("globalsettings.txt")) {
...
case "janitor":
    ...
    <ol>
        ...
        <li>From the page that results, you can aborn any post in the thread, ban a user, or delete the thread itself.
    </ol>
    <?php exit;
case "alladmins":
    if ($mylevel < 9000) fancydie("You don't have permission for that!");?>
    ...
    <table>
        ...
        foreach ($admin as $line) {
            ...
            rtrim($level);
            if ($level > 9000 && $mylevel < 9999) echo "<td></form>$level</td><td>N/A</td></tr>";
...
// Capcode
    ...
    if (!$code) $code = "<b style='color:#f00'>$_COOKIE[adminname]</b>" ?><?=
    $code?>
...
case "password": //pass ?>
    ...
    <form action='admin.php' method='POST'><h2>Current Password</h2>
        For security: <input type="password" name="adminpass"><br>
    ...
    <form action="admin.php" method="POST"><h2>Basic Settings</h2>
        ...
        Skin: <select name="skin"><?php
            ...
            foreach ($board as $tmp) {
                $name = file_get_contents("skin/$tmp/name.txt");
        ...
        <br>Additional threads linked in front page table: <input name="additionalthreads"
                                                                  value="<?= $SETTING[additionalthreads] ?>" size="5">
...
case "ban":
    ...
    <form action="admin.php" method="post"><p>
            ...
            <input type="submit" value="Confirm!">
            <br><input type="checkbox" name="message"> Leave a <b style='color:red'>(USER WAS BANNED FOR THIS POST)</b>
            message?
            <input type="hidden" name="bbs" value="<?= $bbs ?>">
            <input type="hidden" name="dat" value="<?= $key ?>">
            <input type="hidden" name="id" value="<?= $id ?>">
