...
function fancydie($m) {
    ...
    <link rel="stylesheet" href="<?php echo $setting[urltoforum]; ?>skin/2ch/style.css">
    <table border="1" cellspacing="7" cellpadding="3" width="95%" bgcolor="#CCFFCC" align="center" class="mono">
    <tr>
        <td>
            <h1>VIP transmission error!</h1><?= $m ?>
            <hr><?= $_POST[mesg] ?>
            <hr>
...
if ($_POST[pass]) {
    ...
    foreach ($admin as $line) {
        ...
        if (strtolower($_POST[name]) == $name) {
            if (md5($_POST[pass]) != $pass) fancydie("The password you supplied for that username is incorrect.");
            $loggedin = true;
            break;
        }
    }
    if ($loggedin == false) fancydie("You gave a password but your name doesn't match any registered user.");
...
// ID hash 
    $idcrypt = " ";
    if ($setting[haship]) {
        $idcrypt .= "ID: " . substr(base64_encode(pack("H*", sha1($_SERVER[REMOTE_ADDR] . date("d") . SecureSalt()))), 1, 8) . " ";
    }
